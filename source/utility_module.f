...
c     also open atomic VIRIAL file - r. darkins
          
          if(lvirial)then
             open(nvirf,file='VIRIAL',position='append')
          endif
...
c     dump atomic virial header - r. darkins
          
          if(lvirial)then
             write(nvirf,'(a8,4i10,f12.6)') 'timestep',
     x            nstep,natms,keytrj,imcon,tstep
             write(nvirf,'(6a12)')
     x            'xx',
     x            'xy',
...
c     dump atomic virial components to VIRIAL file - r. darkins

            if(lvirial)then
               
               write(nvirf,'(1p,6e12.4)')
     x              atmvir(1,i),
     x              atmvir(2,i),
     x              atmvir(3,i),
     x              atmvir(4,i),
