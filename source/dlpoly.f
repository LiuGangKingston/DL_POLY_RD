...
CBOMB      call bomb(idnode,2008,6,30)
      ...
      allocate (rdbuf(natms),stat=memr)
...
c     initialise atomic virial - r. darkins

      if(lvirial) 
     x     call init_atomicvirial(natms)
...
c     reset atomic virials to zero
c     and switch on/off the atomic virial computation
c     and compute kinetic contribution - r. darkins

        if(lvirial)then
           
           call reset_atomicvirial(natms)
           lvirupdate=ltraj.and.nstep.ge.nstraj.and.
     x          mod(nstep-nstraj,istraj).eq.0
           ...
           do i=iatm0,iatm1
              ...
              thisvir(1)=-weight(i)*vxx(i)*vxx(i)*0.5d0
              thisvir(2)=-weight(i)*vxx(i)*vyy(i)*0.5d0
              thisvir(3)=-weight(i)*vxx(i)*vzz(i)*0.5d0
              thisvir(4)=-weight(i)*vyy(i)*vyy(i)*0.5d0
              thisvir(5)=-weight(i)*vyy(i)*vzz(i)*0.5d0
              thisvir(6)=-weight(i)*vzz(i)*vzz(i)*0.5d0
              
              call update_atomicvirial(i,9)
...
c     all-reduce the atomic virials - r. darkins
        
        call gsync()
        if(lvirial.and.mxnode>1)then

           do i=1,6
              rdbuf=0.0d0
              call gdsum(atmvir(i,:),natms,rdbuf)
              do k=1,natms
                 atmvir(i,k)=rdbuf(k)
              enddo
           enddo
...
c     scale the atomic virial units to be in k-atm.Ang^3 - r. darkins

        if(lvirial.and.idnode.eq.0) call units_atomicvirial(natms)
...
c     deallocate atomic virial memory - r. darkins

      if(lvirial)
     x     call cleanup_atomicvirial
